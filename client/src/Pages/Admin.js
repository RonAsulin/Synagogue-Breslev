...
function Admin(props) {
  ...
  const uploadPdf = async () => {
    ...
    if (res instanceof Error) {
      ...
      if (resEntry instanceof Error) {
        ...
        if (arrHolder.length === 1) setPdfList(arrHolder);
        else setPdfList([resEntry, ...pdfList]);
      }
    }
  ...
  const deletePdf = async (pdf) => {
    ...
    else setPdfList(pdfList.filter((paper) => paper._id !== pdf._id));
  };
  ...
  const handleEditMessage = async (index, updatedMessage) => {
    try {
      ...
      const res = await cl.UpdateMessage(
        messages[index].id,
        updatedMessageData
      );
      if (res instanceof Error) {
  ...
  const uploadPdfDeadLines = async () => {
    ...
    if (res instanceof Error) {
      ...
      if (resEntry instanceof Error) {
        ...
        alert("good");
        arrHolder[0] = resEntry;
        if (arrHolder.length === 1) setPdfListDL(arrHolder);
        else setPdfListDL([resEntry, ...pdfListDL]);
      }
    }
    ...
    <div className="main-container">
      ...
      <Container className="text-center mt-5">
        <Row className="justify-content-end">
          ...
          <Col xs="auto">
            <h3
              style={{ color: "black", marginBottom: "1rem", marginLeft: 100 }}
            >
              להעלות עלון חדש
            </h3>
            <hr className="mb-0.5" />
            <div>
              ...
              <p style={{ marginTop: 10, fontWeight: "bolder" }}>
                למחיקת עלון בחר עלון מהרשימה
              </p>
            </div>
          </Col>
        </Row>
        ...
        <Row className="justify-content-end">
          ...
          <Col xs="auto">
            ...
            <div>
              <input
                type="file"
                onChange={(event) => setPdfUploadDL(event.target.files[0])}
                className="form-control-file"
              />
              <button
                onClick={uploadPdfDeadLines}
                variant="success"
                className="mt-4 btn-custom"
                disabled={isLoading}
              >
              ...
              {isMessageOpen && (
                <CardContent>
                  ...
                  <div className="button-row">
                    {" "}
                    {/* Move this div to create a separate row */}
                    <Button
