...
function Dashboard() {
  ...
  const handleOffcanvasSelect = (selectedPage) => {
    setOffcanvasShown(false); // Close the Offcanvas when a page is selected
    setView(selectedPage); // Set the selected page
  };
  ...
  const fetchStorage = () => {
    ...
    if (item) setIsAdmin(true);
  };

  const fetch = () => {
    const token = localStorage.getItem("token");
    axios
      .get(baseURL + "/account/admin", {
  ...
  const loginModal = () => {
    return (
      <Modal show={modalShown} onHide={() => setModalShwown(false)}>
        <Modal.Header>
        ...
        <Navbar expand="sm" className="bg-body-tertiary mb-0">
          ...
          <Container fluid>
            ...
            <Navbar.Toggle
              ...
              onClick={() => setOffcanvasShown(!offcanvasShown)}
            />
            <Navbar.Offcanvas
              show={offcanvasShown}
            ...
            >
              ...
              <Offcanvas.Body>
                ...
                >
                  {isAdmin && (
                    <Nav.Link onClick={() => handleOffcanvasSelect("admin")}>
                      Management
                    </Nav.Link>
                  )}
                  <Nav.Link onClick={() => handleOffcanvasSelect("main")}>
                    דף ראשי
                  </Nav.Link>
                  <Nav.Link
                    onClick={() => handleOffcanvasSelect("prayingTimes")}
                  >
                    זמני תפילות
                  </Nav.Link>
                  <Nav.Link onClick={() => handleOffcanvasSelect("message")}>
                    הודעות
                  </Nav.Link>
                  <Nav.Link onClick={() => handleOffcanvasSelect("papers")}>
                    עלונים
                  </Nav.Link>
                  <Nav.Link onClick={() => handleOffcanvasSelect("deadlines")}>
                    מועדים
                  </Nav.Link>
                  <Nav.Link onClick={() => handleOffcanvasSelect("donation")}>
                    תרומות
                  </Nav.Link>
                  <Nav.Link onClick={() => handleOffcanvasSelect("about")}>
                    אודות
                  </Nav.Link>
                ...
                >
                  ...
                  <Button
                    ...
                    className={
                      isAdmin ? "admin-button-active" : "admin-button-inactive"
                    }
                    onClick={async () => {
                      if (!isAdmin) setModalShwown(true);
                      else {
                        const cl = FirebaseClient.getInstance();
                        await cl.logout();
                        setIsAdmin(!isAdmin);
